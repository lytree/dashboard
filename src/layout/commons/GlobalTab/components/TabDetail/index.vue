<template>
  <div ref="tabRef" class="flex items-end h-full">
    <chrome-tab v-for="(item, index) in tab.tabs" :key="item.path" :is-active="tab.activeTab === item.path"
      :primary-color="theme.themeColor" :closable="item.path !== tab.homeTab.path" :dark-mode="theme.darkMode"
      :class="{ '!mr-0': index === tab.tabs.length - 1 }" :is-last="index === tab.tabs.length - 1"
      @click="tab.handleClickTab(item.path)" @close="tab.removeTab(item.path)"
  ...
  <context-menu v-model:visible="dropdown.visible" :current-path="dropdown.currentPath" :x="dropdown.x"
    :y="dropdown.y" />
</template>
